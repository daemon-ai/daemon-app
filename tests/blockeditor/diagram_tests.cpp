...
void DiagramTests::detectsFamily()
{
    ...
    QCOMPARE(detectFamily(QStringLiteral("   %% comment\nflowchart TD")), QStringLiteral("flowchart"));
}

void DiagramTests::parsesNodesAndShapes()
{
    const DiagramModel m = parseFlowchart(QStringLiteral(
        "flowchart TD\n"
        "A[Rect]\n"
        "B(Round)\n"
        "C{Diamond}\n"
...
void DiagramTests::parsesSubgraphsAndClassDef()
{
    ...
    QCOMPARE(m.classDefs[QStringLiteral("hot")].value(QStringLiteral("fill")), QStringLiteral("#f00"));
...
void DiagramTests::layoutAssignsRanks()
{
    ...
    const DiagramModel m = engine.buildModel(
...
void DiagramTests::edgeLabelReservesSpace()
{
    ...
    const DiagramModel plain = engine.buildModel(
        QStringLiteral("flowchart TD\nA-->C\nB-->C\n"), Style{}, 600.0);
