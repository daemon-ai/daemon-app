...
MockCronStore::MockCronStore(QObject* parent)
    : ICronStore(parent)
    , m_jobs(new uimodels::VariantListModel(this))
{
...
QObject* MockCronStore::jobs() const { return m_jobs; }
                                 ...
                                 const QString& profile)
{
    ...
    m_jobs->upsert(mk(id, name.isEmpty() ? QStringLiteral("New job") : name,
                      ...
                      profile.isEmpty() ? QStringLiteral("General Assistant") : profile,
                      QString(), QStringLiteral("pending"), QStringLiteral("—"), true));
    save();
    emit changed();
    return id;
}
