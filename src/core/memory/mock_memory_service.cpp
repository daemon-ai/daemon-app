...
MockMemoryService::MockMemoryService(QObject* parent)
    : IMemoryService(parent)
{
...
void MockMemoryService::seed()
{
    ...
    static const Spec specs[] = {
        ...
        { "m16", "Consolidated summary of the June planning session.", "consolidation",
          "ga-plan", "session", "episodic", 1, 0.9, "stated", 3, "2026-06-22T20:00:00Z", "",
          "prof-1" },
        ...
        { "m06", "SQLite backs the Mnemosyne bank with WAL mode.", "tool", "cd-api",
          "global", "episodic", 2, 0.55, "inferred", 0, "2026-05-30T11:00:00Z", "", "prof-2" },
    ...
    for (const Spec& s : specs) {
        ...
        e.status = e.supersededBy.isEmpty() ? QStringLiteral("active")
...
MemoryGraph MockMemoryService::buildAssociation(const QString& seedId, int depth, int limit) const
{
    ...
    for (const auto& ref : m_refs) {
        if (present.contains(ref.first) && present.contains(ref.second))
            g.edges.append(MemoryEdge{ ref.first, ref.second, QStringLiteral("references"), 0.8,
                                       QString() });
    }
    return g;
}
...
MemoryGraph MockMemoryService::buildConstellation(int limit) const
{
    ...
    for (const auto& ref : m_refs) {
        if (present.contains(ref.first) && present.contains(ref.second))
            g.edges.append(MemoryEdge{ ref.first, ref.second, QStringLiteral("references"), 0.6,
                                       QString() });
    }
    g.clusters.append(MemoryCluster{ QStringLiteral("memories"), QStringLiteral("Memories"),
                                     memoryIds });
    g.clusters.append(MemoryCluster{ QStringLiteral("entities"), QStringLiteral("Entities"),
                                     entityIds });
    return g;
}
...
void MockMemoryService::requestStats()
{
    ...
    QMetaObject::invokeMethod(
        this, [this, s] { emit statsReady(s); }, Qt::QueuedConnection);
}
...
void MockMemoryService::requestGraph(const QString& kind, const QString& seed, int depth, int limit)
{
    MemoryGraph g;
    ...
    QMetaObject::invokeMethod(
        this, [this, g] { emit graphReady(g); }, Qt::QueuedConnection);
}
