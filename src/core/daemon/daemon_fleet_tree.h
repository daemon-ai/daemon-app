...
// rows ({ depth, id, name, kind, status, model, engine, engineAgent, lifetime, sessionId, role });
// seeds from the cache in the ctor so the tree renders last-known with no connection, and rebuilds
// on each treeRefreshed. pause/resume/scale issue the wire control op via the repository; the
// operator Steer/Cancel row actions (F4) go through ISessionRoster with the row's sessionId.
//
// [wave2:app-delegation] Engine identity + lifetime (F3): each row carries the authoritative
// per-child wire fields (UnitNode v29: engine = "Core" | "Foreign" + engineAgent, lifetime =
// "Persistent" | "Ephemeral") decoded straight off the Tree response. This replaces the wave-1
// client-side profile-join (the node peer is v29, so the wire always answers directly).
//
