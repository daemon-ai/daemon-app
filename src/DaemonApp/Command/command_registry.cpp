...
CommandRegistry::CommandRegistry(QObject* parent) : QAbstractListModel(parent) {
    ...
    m_all = {
        {QStringLiteral("new"), tr("New session"), tr("Session"), tr("Start a fresh chat"),
         QStringLiteral("Ctrl+T")},
        {QStringLiteral("search"), tr("Search sessions"), tr("Navigation"), tr("Jump to a session"),
         QString()},
        {QStringLiteral("settings"), tr("Open settings"), tr("Navigation"), QString(), QString()},
        ...
        {QStringLiteral("clear"), tr("Clear session"), tr("Session"), tr("Remove all messages"),
         QString()},
...
QHash<int, QByteArray> CommandRegistry::roleNames() const {
    return {
        {IdRole, "commandId"}, {TitleRole, "title"},       {GroupRole, "group"},
        {HintRole, "hint"},    {ShortcutRole, "shortcut"},
    };
}
