...
void CoordinateMap::rebuild(const QString &text)
{
    ...
    for (qsizetype utf16Offset = 0; utf16Offset < m_text.size();) {
        ...
        const bool surrogatePair = current.isHighSurrogate()
            && utf16Offset + 1 < m_text.size()
            && m_text.at(utf16Offset + 1).isLowSurrogate();
        const qsizetype utf16Width = surrogatePair ? 2 : 1;
        const qsizetype utf8Width = QStringView(m_text).mid(utf16Offset, utf16Width).toUtf8().size();
...
LineColumn CoordinateMap::utf16ToLineColumn(qsizetype utf16Offset) const
{
    ...
    const auto it = std::upper_bound(m_lineStartsUtf16.cbegin(), m_lineStartsUtf16.cend(), utf16Offset);
    const qsizetype line = std::max<qsizetype>(0, std::distance(m_lineStartsUtf16.cbegin(), it) - 1);
...
qsizetype CoordinateMap::lineColumnToUtf16(qsizetype line, qsizetype column) const
{
    ...
    const qsizetype nextLineStart = line + 1 < m_lineStartsUtf16.size() ? m_lineStartsUtf16[line + 1] : m_text.size() + 1;
