...
BlockProjection InlineProjector::project(const BlockRecord& block, bool revealMarkdown,
    ...
    } else if (!revealAll && (block.type == BlockType::BulletListItem ||
    ...
    while (i < contentEnd) {
                        ...
                        if (scan < contentEnd && raw.at(scan) == QLatin1Char('{')) {
                            ...
                            if (attrClose > scan && attrClose < contentEnd) {
                                ...
                                width = imageDimensionValue(
                                    imageAttribute(attrs, QStringLiteral("width")), &widthPct);
                                height = imageDimensionValue(
                                    imageAttribute(attrs, QStringLiteral("height")), &heightPct);
                                if (widthPct) {
        ...
        if (!revealAll && raw.at(i) == QLatin1Char('<')) {
            ...
            if (close > i + 1 && close < contentEnd) {
                ...
                if (!hasWhitespace(inner) && (inner.contains(QStringLiteral("://")) ||
                                              inner.startsWith(QStringLiteral("mailto:")))) {
        ...
        case SpanKind::Link: {
            ...
            if (!span.imageUrl.isEmpty()) {
                inner += QStringLiteral(
                             "<img src=\"%1\" height=\"16\" style=\"vertical-align:middle;\"> ")
                             .arg(be::resolveImageSource(span.imageUrl).toHtmlEscaped());
            }
            inner +=
                QStringLiteral("<span style=\"color:%1; text-decoration:underline;\">%2</span>")
